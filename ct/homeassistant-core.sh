...
function update_script() {
  ...
  UPD=$(whiptail --backtitle "Proxmox Scripts" --title "UPDATE" --radiolist --cancel-button Exit-Script "Spacebar = Select" 11 58 4 \
    "1" "Update Core" ON \
    "2" "Install HACS" OFF \
  ...
  header_info
  if [ "$UPD" == "1" ]; then
    if (whiptail --backtitle "Proxmox Scripts" --defaultno --title "SELECT BRANCH" --yesno "Use Beta Branch?" 10 58); then
      clear
      header_info
